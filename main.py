...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        icon = QtGui.QIcon()
        # Finds the absolute path to the directory this file is in
        current_dir = os.path.dirname(os.path.abspath(__file__))
        # Combines that with your assets folder
...
"selection-background-color: rgb(85, 170, 255);")
    ...
    def open_help(self):
        ...
        html_path = os.path.join(base_dir, "docs", "help.html")
        url = QUrl.fromLocalFile(html_path)
        QDesktopServices.openUrl(url)

    def display_license(self):
        # 1. Locate the directory where this script lives
        base_dir = os.path.dirname(os.path.abspath(__file__))
        
        # 2. Build the path to the license file inside 'assets'
        license_path = os.path.join(base_dir, "license")
        
        # 3. Open and read using the absolute path
        try:
            with open(license_path, 'r', encoding='utf-8') as file:
                data = file.read()
                # ... now display 'data' in your QMessageBox or text widget
